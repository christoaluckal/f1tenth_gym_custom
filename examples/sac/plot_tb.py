...
def generate_pkl_avgs(folder_loc,pkl_name):
    base_dir = folder_loc
    dirs = os.listdir(base_dir)
    dirs = [os.path.join(base_dir, d) for d in dirs if os.path.isdir(os.path.join(base_dir, d))]
    dirs_files = [os.path.join(d, f) for d in dirs for f in os.listdir(d)]
...
def plotavgs(pkl_file,config_s='1'):
    with open(pkl_file, 'rb') as f:
        res_dict = pickle.load(f)
    ...
    for key in res_dict:
        if key[0] == config:
            ...
            res_dict[key] = res_dict[key][:-1]
            for i in range(len(res_dict[key])):
                length = len(res_dict[key][i]['test']['steps'])
                if length < 100:
                    continue
            ...
            plt.plot(steps, smoothed, label=key,color=color_scheme[key[2:]],linewidth=2)
            plt.scatter(steps, smoothed, color=color_scheme[key[2:]],s=10)
...
if args[0] == 'g':
    folder_loc = args[1]
    pkl_name = args[2]
    generate_pkl_avgs(folder_loc,pkl_name)
else:
    pkl_file = args[1]
    config_s = args[2]
    # plot(pkl_file,config_s)
    plotavgs(pkl_file,config_s)